...
void pge_sprite_draw(PGESprite *this, GContext *ctx) {
  graphics_draw_bitmap_in_rect(ctx, this->bitmap, GRect(this->position.x, this->position.y, gbitmap_get_bounds(this->bitmap).size.w, gbitmap_get_bounds(this->bitmap).size.h));
}
...
GRect pge_sprite_get_bounds(PGESprite *this) {
  return GRect(this->position.x, this->position.y, 
      gbitmap_get_bounds(this->bitmap).size.w, gbitmap_get_bounds(this->bitmap).size.h);
}
